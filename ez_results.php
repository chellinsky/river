	...
	class ez_results
	{
		...
		*/
		...
		function build_navigation()
		{
			...
			{				
				$out .= $this->create_link(preg_replace("/\?.*/",'',$_SERVER['PHP_SELF']) .  '?choice='.$_GET['choice'].'&BRSR=0'.$this->qs,$this->text_start_page,$this->text_hover_msg_start,$this->get_style('start_page')).' ';			
			}
			else
			{
			...
			{
				$out .= $this->create_link(preg_replace("/\?.*/",'',$_SERVER['PHP_SELF']) .  '?choice='.$_GET['choice'].'&BRSR='. ($_REQUEST['BRSR']-$this->num_results_per_page) .$this->qs,$this->merge_num('back',$this->num_results_per_page),$this->merge_num('hover_msg_back',$this->num_results_per_page),$this->get_style('back')).' ';
			}
			else
			{
			...
			if ( $this->num_results > $this->num_results_per_page )
			{
				...
				for ( $i=($this->cur_page_set*$this->num_browse_links); $i < ($this->cur_page_set*$this->num_browse_links)+$this->num_browse_links; $i++ )
				{
					if ( ($i*$this->num_results_per_page) < $this->num_results )
					{
						...
						{
							$out .= $this->create_link(preg_replace("/\?.*/",'',$_SERVER['PHP_SELF']) .  '?choice='.$_GET['choice'].'&BRSR='. ($i*$this->num_results_per_page) .$this->qs,($i+1),$this->merge_num('hover_msg_link',($i+1)),$this->get_style('link')).' ';
						}
					}		
				}
			...
			{
				$out .= $this->create_link(preg_replace("/\?.*/",'',$_SERVER['PHP_SELF']) .  '?choice='.$_GET['choice'].'&BRSR='. ($_REQUEST['BRSR']+$this->num_results_per_page) .$this->qs,$this->merge_num('next',$this->num_results_per_page),$this->merge_num('hover_msg_next',$this->num_results_per_page),$this->get_style('next')).' ';
			}
			else
			{
				if ( $this->num_pages >= $this->num_browse_links )
			...
			{
				$out .= $this->create_link(preg_replace("/\?.*/",'',$_SERVER['PHP_SELF']) .  '?choice='.$_GET['choice'].'&BRSR='. (($this->num_pages*$this->num_results_per_page)-$this->num_results_per_page) .$this->qs,$this->merge_num('last_page',$this->num_pages),$this->merge_num('hover_msg_end',$this->num_pages),$this->get_style('last_page')).' ';
			}
			else
			{
